// SPDX-License-Identifier: MIT
pragma solidity >=0.8.25;

import {Test} from "forge-std/Test.sol";
import {BLS12381Lib} from "@src/BLS12381Lib.sol";

contract BLS12381LibTest is Test {
    using BLS12381Lib for BLS12381Lib.G1Point;
    using BLS12381Lib for BLS12381Lib.G2Point;
    function test_mulBaseG1_identity() public view {
        BLS12381Lib.G1Point memory result = BLS12381Lib.mulBaseG1(1);
        assertEq(result.asBytes(), BLS12381Lib.G1_GENERATOR);
    }

    function test_mulBaseG1_zero() public view {
        BLS12381Lib.G1Point memory zeroResult = BLS12381Lib.mulBaseG1(0);
        bytes memory infinity = new bytes(128);
        assertEq(zeroResult.asBytes(), infinity);
    }

    function test_mulBaseG1_scalar() public view {
        BLS12381Lib.G1Point memory point = BLS12381Lib.mulBaseG1(123456789);
        assertEq(point.asBytes(), hex"");
    }

    function test_mulBaseG2_identity() public view {
        BLS12381Lib.G2Point memory result = BLS12381Lib.mulBaseG2(1);
        assertEq(result.asBytes(), BLS12381Lib.G2_GENERATOR);
    }

    function test_mulBaseG2_zero() public view {
        BLS12381Lib.G2Point memory zeroResult = BLS12381Lib.mulBaseG2(0);
        bytes memory infinity = new bytes(256);
        assertEq(zeroResult.asBytes(), infinity);
    }

    function test_mulBaseG2_scalar() public view {
        BLS12381Lib.G2Point memory point = BLS12381Lib.mulBaseG2(123456789);
        assertEq(point.asBytes(), hex"");
    }
}
